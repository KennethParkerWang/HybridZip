...
| E5 | K=2/K=4/K=8 versus full Auto | Report complete-byte regret, selected-mode coverage, and a matching 32 KiB forced-mode tie-aware oracle | Runtime complete at 432/432 rows; full-Auto reference, K=2/K=4/K=8 ablations, and 36 matching 32 KiB forced-oracle rows validated; held-out promotion remains pending |
...
| F3 | Block executor | Canonical archive order and byte-exact repeats; measure before/after Fast throughput | Fast-only executor/1 KiB gate and post-change single-worker E6 timing passed; second worker-count scaling comparison remains optional |
...
held-out regret remain F1 acceptance requirements. Fast K=4 and the append-only
Mode-43 extension are implemented, and the current single-worker corpus-level
throughput matrix is complete. Fast now has a bounded independent-block thread
...
the complete forced ledger and E5 runtime package are available, while fitted
ranker weights and held-out validation remain pending.
