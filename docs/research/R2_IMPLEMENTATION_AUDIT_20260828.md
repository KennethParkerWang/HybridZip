...
| `fast` | 1,024 bytes | mode 2/zstd; 662-byte archive; exact decode |
| HZ01 | 1,024 bytes | 537-byte archive; exact decode |

`hz_structure_routing_tests.exe` and `hz_r2_codec_tests.exe` also completed
on this working tree. No complete CTest matrix was run.
