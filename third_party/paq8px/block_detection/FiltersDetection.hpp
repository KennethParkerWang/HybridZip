...
#include "../record_model/Array.hpp"
#include "../record_model/BlockType.hpp"
#include "../record_model/CharacterNames.hpp"
#include "../record_model/Hash.hpp"
#include "../record_model/file/File.hpp"
#include "../record_model/Utils.hpp"
#include "../record_model/filter/TextParserStateInfo.hpp"
#include <cctype>
#include <cstdint>
#include <cstring>

// HybridZip retains the complete donor Filters.hpp beside this adapted copy.
// This compilation unit uses only the detection half and keeps detection state
// local to one call. Optional zlib recursion and TAR transformation are outside
// the SSE block-profile contract and remain available in the upstream copy.
#define DISABLE_ZLIB
...
static DetectionInfo detect(File *in, uint64_t blockSize, const TransformOptions* const transformOptions) {
  ...
  int gifplt = 0;
  bool gifGray = false;
  BlockType dett = BlockType::DEFAULT;
  ...
  for (uint64_t i = 0; i < n; ++i) {
    ...
    // CD sectors are a transform-only family and do not feed an SSE
    // specialist. The complete donor branch remains in the upstream copy.
#if 0
    ...
    // UStar detection belongs to the donor's recursive transform path. The
    // complete branch is retained in upstream/filter/Filters.hpp; this raw SSE
    // profile adapter deliberately leaves TAR bytes in the Generic family.
...
#if 0  // HybridZip compiles detector code only; full filter pipeline is retained above.
#include "bmp.hpp"
