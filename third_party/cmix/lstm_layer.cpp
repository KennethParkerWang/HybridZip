...
float LstmLayer::Rand(std::uint64_t& state) {
  state += 0x9E3779B97F4A7C15ULL;
  std::uint64_t value = state;
  value = (value ^ (value >> 30U)) * 0xBF58476D1CE4E5B9ULL;
  value = (value ^ (value >> 27U)) * 0x94D049BB133111EBULL;
  value ^= value >> 31U;
  const std::uint32_t mantissa = static_cast<std::uint32_t>(value >> 40U);
...
LstmLayer::LstmLayer(unsigned int input_size, unsigned int auxiliary_input_size,
    ...
    float gradient_clip, float learning_rate, std::uint64_t& random_state) :
  ...
  for (unsigned int i = 0; i < num_cells_; ++i) {
    for (unsigned int j = 0; j < forget_gate_.weights_[i].size(); ++j) {
      forget_gate_.weights_[i][j] = low + Rand(random_state) * range;
      input_node_.weights_[i][j] = low + Rand(random_state) * range;
      output_gate_.weights_[i][j] = low + Rand(random_state) * range;
    }
