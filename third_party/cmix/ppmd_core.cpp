// PPMd was written by Dmitry Shkarin.
// mod_ppmd was adapted from PPMd by Eugene Shelwien.
// The cmix implementation was adapted from mod_ppmd_v2:
// http://encode.su/threads/2515-mod_ppmd
//
// HybridZip adaptation source: cmix v21, src/models/ppmd.cpp, released by
// Byron Knoll on 2024-09-10. HybridZip separates probability queries from
// byte observation and accepts allocator capacity in bytes. This file remains
// licensed under GNU GPL v3; see COPYING in this directory.
...
static constexpr std::int8_t EscCoef[12] = {
...
BLK_NODE BList[N_INDEXES+1];
  ...
  uint GlueCount = 0;
  uint GlueCount1 = 0;
  qword SubAllocatorSize = 0;
  byte* pText = nullptr;
  byte* UnitsStart = nullptr;
  byte* LoUnit = nullptr;
  byte* HiUnit = nullptr;
  byte* AuxUnit = nullptr;

uint U2B( uint NU ) {
  return 8*NU+4*NU;
}

int StartSubAllocator( qword size_bytes ) {
  HeapStart = new (std::nothrow) byte[static_cast<std::size_t>(size_bytes)];
  if( HeapStart==nullptr ) return 0;
  SubAllocatorSize = size_bytes;
  return 1;
}
...
void ExpandTextArea() {
  ...
  if( AuxUnit!=UnitsStart ) {
    uint stamp = 0;
    std::memcpy(&stamp, AuxUnit, sizeof(stamp));
    if( stamp != ~uint(0) )
...
struct PPM_CONTEXT {
  ...
  STATE& oneState() { return data.one; }
};

static_assert(sizeof(STATE) == 6, "PPMd state layout must remain six bytes");
static_assert(sizeof(MULTI_STATE) == 6,
              "PPMd multi-state layout must remain six bytes");
static_assert(sizeof(PPM_CONTEXT) == 12,
              "PPMd context layout must remain one allocator unit");

STATE* getStats( PPM_CONTEXT* This ) {
  return (STATE*)Indx2Ptr(This->data.many.iStats);
}
...
PPM_CONTEXT* suff( PPM_CONTEXT* This ) { return (PPM_CONTEXT*)Indx2Ptr(This->iSuffix); }
  ...
  PPM_CONTEXT* MaxContext = nullptr;
...
STATE* rescale( PPM_CONTEXT& q, int OrderFall, STATE* FoundState ) {
  ...
  int a, i;
  int f0 = p->Freq;
  int sf = q.data.many.SummFreq;
  int EscFreq = sf-f0;
  q.data.many.SummFreq = p->Freq = (f0+of)>>1;
  ...
  if( p->Freq==0 ) {
    ...
    q.data.many.iStats =
        Ptr2Indx( ShrinkUnits(getStats(&q),a,(q.NumStats+2)>>1) );
  }

  q.data.many.SummFreq += (EscFreq+1) >> 1;
  if( OrderFall || (q.Flags & 0x04)==0 ) {
    a = (sf-=EscFreq) - f0;
    a = CLAMP(
        uint((f0*q.data.many.SummFreq - sf*getStats(&q)->Freq + a-1) / a),
        2U, MAX_FREQ/2U-18U);
  } else {
    a = 2;
  }
...
uint cutOff( PPM_CONTEXT& q, int Order, int MaxOrder ) {
  ...
  if( q.NumStats==0 ) {
    ...
    q.data.many.iStats = Ptr2Indx(p0);
    ...
    if( i!=q.NumStats && Order>0 ) {
      ...
      if( i==0 ) {
        ...
        p[0].Freq =
            1+(2*(p[0].Freq-1))/(q.data.many.SummFreq-p[0].Freq);
        q.oneState() = p[0];
        FreeUnits( p, tmp );
      } else {
        p = (STATE*)ShrinkUnits( p0, tmp, (i+2)>>1 );
        q.data.many.iStats = Ptr2Indx(p);
        Scale = (q.data.many.SummFreq>16*i);
        q.Flags = (q.Flags & (0x10+0x04*Scale));
        if( Scale ) {
          EscFreq = q.data.many.SummFreq;
          q.data.many.SummFreq = 0;
          for( i=0; i<=q.NumStats; i++ ) {
            EscFreq -= p[i].Freq;
            p[i].Freq = (p[i].Freq+1)>>1;
            q.data.many.SummFreq += p[i].Freq;
            q.Flags |= 0x08*(p[i].Symbol>=0x40);
          };
          EscFreq = (EscFreq+1)>>1;
          q.data.many.SummFreq += EscFreq;
        } else {
...
void StartModelRare( void ) {
  ...
  InitRL = static_cast<uint>(-((_MaxOrder<13) ? _MaxOrder : 13));
  RunLength = InitRL;

  MaxContext = (PPM_CONTEXT*)AllocContext();
  MaxContext->NumStats = 255;
  MaxContext->data.many.SummFreq = 255+2;
  MaxContext->data.many.iStats = Ptr2Indx(AllocUnits(256/2));
  MaxContext->Flags = 0;
...
PPM_CONTEXT* UpdateModel( PPM_CONTEXT* MinContext ) {
  ...
  *pText++ = FSymbol;
  ...
  s0 = MinContext->data.many.SummFreq - FFreq;
  ...
  for( pc=MaxContext; pc!=MinContext; pc=suff(pc) ) {
    ...
    if( ns1 ) {
      ...
      if( ns1&1 ) {
        ...
        pc[0].data.many.iStats = Ptr2Indx(p);
      }

      pc[0].data.many.SummFreq += QTable[ns+4] >> 3;
    } else {
      ...
      pc[0].data.many.SummFreq =
          p[0].Freq + (ns>1) + ExpEscape[QTable[BSumm>>8]];
    }

    cf = (FFreq-1)*(5 + pc[0].data.many.SummFreq);
    sf = s0 + pc[0].data.many.SummFreq;

    if( cf<=3*sf ) {

      cf = 1 + (2*cf>sf) + (2*cf>3*sf);
      pc[0].data.many.SummFreq += 4;
    } else {
...
NO_LOOP:
  ...
  if( pc[0].NumStats ) {
    for( p=getStats(pc); p[0].Symbol!=sym; p++ );
    cf = p[0].Freq - 1;
    s0 = pc[0].data.many.SummFreq - pc[0].NumStats - cf;
...
void processSymbol1( PPM_CONTEXT& q, int symbol ) {
  ...
  int low = 0;
  int freq = p[0].Freq;
  int total = q.data.many.SummFreq;
  int flag;
...
void processSymbol2( PPM_CONTEXT& q, int symbol ) {
  ...
  if( cnum != 0xFF ) {
    ...
    psee2c+= (q.data.many.SummFreq > 10*(cnum+1));
...
void processSymbol2_T( PPM_CONTEXT& q, int ) {
  ...
  uint Init( uint MaxOrder, qword memory_bytes, uint CutOff ) {
    _MaxOrder = static_cast<int>(MaxOrder);
    _CutOff = CutOff;

    PPMD_STARTUP();

    if( !StartSubAllocator( memory_bytes ) ) return 1;
...
struct PpmdCore::Impl {
  ppmd_Model model;
  WeightArray cached_prediction{};
  bool prediction_valid = false;
  bool preparation_needed_before_observe = false;

  void prepare_prediction() {
    model.ppmd_PrepareByte();
    for (std::size_t symbol = 0; symbol < PpmdCore::kSymbolCount; ++symbol) {
      const uint weight = model.sqp[symbol];
      if (weight == 0) {
        throw std::runtime_error("PPMd disabled a byte symbol");
      }
      cached_prediction[symbol] = weight;
    }
    prediction_valid = true;
    preparation_needed_before_observe = false;
  }
};

PpmdCore::PpmdCore(const int max_order, const std::size_t memory_bytes)
    : impl_(std::make_unique<Impl>()) {
  if (max_order < 2 || max_order > ORealMAX) {
    throw std::invalid_argument("PPMd order must be in [2, 256]");
  }
  constexpr std::size_t kMinimumMemoryBytes = 1U << 20U;
  if (memory_bytes < kMinimumMemoryBytes) {
    throw std::invalid_argument("PPMd memory must be at least 1 MiB");
  }
  if (memory_bytes > std::numeric_limits<uint>::max()) {
    throw std::invalid_argument("PPMd memory must not exceed 4 GiB - 1 byte");
  }
  if (impl_->model.Init(static_cast<uint>(max_order),
                        static_cast<qword>(memory_bytes), 1) != 0) {
    throw std::bad_alloc();
  }
}

PpmdCore::~PpmdCore() = default;
PpmdCore::PpmdCore(PpmdCore&&) noexcept = default;
PpmdCore& PpmdCore::operator=(PpmdCore&&) noexcept = default;

void PpmdCore::reset() {
  impl_->model.Reset();
  impl_->prediction_valid = false;
  impl_->preparation_needed_before_observe = false;
}

void PpmdCore::predict(WeightArray& out) {
  if (!impl_->prediction_valid) {
    impl_->prepare_prediction();
  }
  out = impl_->cached_prediction;
}

void PpmdCore::observe(const std::uint8_t symbol) {
  // cmix prepares after every byte. If callers skipped predict(), perform the
  // same mask-epoch transition before applying the next observation.
  if (impl_->preparation_needed_before_observe) {
    impl_->prepare_prediction();
  }
  impl_->model.ppmd_UpdateByte(symbol);
  impl_->prediction_valid = false;
  impl_->preparation_needed_before_observe = true;
}

std::size_t PpmdCore::context_depth() const noexcept {
  const int depth = impl_->model._MaxOrder - impl_->model.OrderFall;
  return depth > 0 ? static_cast<std::size_t>(depth) : 0U;
}
