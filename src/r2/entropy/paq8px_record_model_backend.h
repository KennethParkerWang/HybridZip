...
// Complete PAQ8px RecordModel prediction graph adapted to HZ02's bit stream.
class Paq8pxRecordModelBackend final : public IBlockEntropyBackend {
public:
    const char* name() const noexcept override { return "paq8px-record-model"; }
    EntropyKind kind() const noexcept override {
        return EntropyKind::Paq8pxRecordModel;
    }
