...
constexpr std::array<std::uint8_t, 4> kPayloadMagic{'H', 'Z', 'P', '8'};
constexpr std::uint8_t kPayloadVersion = 1;
constexpr std::uint8_t kPayloadFlags = 1;  // Donor end marker is required.
...
constexpr std::size_t kEnvelopeSizeOffset = 28;
constexpr std::size_t kReservedOffset = 29;
constexpr std::size_t kOutputChunkSize = 64U * 1024U;
// PPMdI can visit up to order + 1 contexts per symbol. Forty bytes per input
// symbol plus fixed slack bounds callback output without preallocating it.
constexpr std::size_t kMaximumExpansionFactor = 40;
constexpr std::size_t kRangeCoderSlack = 128;
...
std::size_t stream_capacity_for(const std::size_t input_size) {
    const std::size_t expanded = checked_multiply(
        input_size, kMaximumExpansionFactor, "PPMd8 stream bound overflow");
    return checked_add(expanded, kRangeCoderSlack,
                       "PPMd8 stream bound overflow");
}
...
class Ppmd8Model {
public:
    explicit Ppmd8Model(const std::uint32_t memory_size) {
        Ppmd8_Construct(&model_);
        if (!Ppmd8_Alloc(&model_, memory_size, &g_Alloc)) {
            throw std::bad_alloc();
        }
    }

    ~Ppmd8Model() { Ppmd8_Free(&model_, &g_Alloc); }

    Ppmd8Model(const Ppmd8Model&) = delete;
    Ppmd8Model& operator=(const Ppmd8Model&) = delete;

    CPpmd8* get() noexcept { return &model_; }
...
void validate_profile(const unsigned order,
                      ...
                      const unsigned restore_method) {
    if (order < PPMD8_MIN_ORDER || order > PPMD8_MAX_ORDER) {
        throw std::invalid_argument("PPMd8 order must be 2 through 16");
    }
    if (memory_size < Ppmd8Backend::kMinimumMemorySize ||
        memory_size > Ppmd8Backend::kMaximumMemorySize) {
        throw std::invalid_argument("PPMd8 model memory is outside limits");
    }
    if (restore_method > PPMD8_RESTORE_METHOD_CUT_OFF) {
        throw std::invalid_argument("PPMd8 restore method is unsupported");
    }
}

}  // namespace

Ppmd8Backend::Ppmd8Backend(const unsigned order,
                           const std::uint32_t memory_size,
                           const unsigned restore_method,
                           const std::size_t maximum_output_size)
    : order_(order),
      memory_size_(memory_size),
      restore_method_(restore_method),
      maximum_output_size_(maximum_output_size) {
    validate_profile(order_, memory_size_, restore_method_);
    if (maximum_output_size_ > std::numeric_limits<std::uint32_t>::max()) {
        throw std::invalid_argument("PPMd8 output limit exceeds payload format");
    }
}

std::vector<std::uint8_t> Ppmd8Backend::encode(const ByteView input) const {
    require_valid_view(input, "PPMd8 encoder");
    if (input.size() > maximum_output_size_ ||
        input.size() > std::numeric_limits<std::uint32_t>::max()) {
        throw std::length_error("PPMd8 input exceeds configured block limit");
    }

    Ppmd8Model model(memory_size_);
    ChunkedByteOut stream = make_output(stream_capacity_for(input.size()));
    model.get()->Stream.Out = &stream.vt;
    Ppmd8_Init_RangeEnc(model.get());
    Ppmd8_Init(model.get(), order_, restore_method_);
    for (std::size_t index = 0; index < input.size(); ++index) {
        Ppmd8_EncodeSymbol(model.get(), input[index]);
    }
    Ppmd8_EncodeSymbol(model.get(), PPMD8_SYM_END);
    Ppmd8_Flush_RangeEnc(model.get());
    if (stream.failed) {
        throw std::length_error("PPMd8 range stream exceeded safety bound");
    }
    if (stream.size > std::numeric_limits<std::uint32_t>::max()) {
        throw std::length_error("PPMd8 range stream exceeds payload format");
    }

    std::vector<std::uint8_t> output(
        checked_add(kPayloadHeaderSize, stream.size,
                    "PPMd8 payload size overflow"));
    ...
    output[7] = static_cast<std::uint8_t>(restore_method_);
    ...
    write_u32_le(output, kCompressedCrcOffset,
                 crc32(ByteView(output.data() + kPayloadHeaderSize,
                                stream.size)));
    output[kEnvelopeSizeOffset] =
        static_cast<std::uint8_t>(kPayloadHeaderSize);
    return output;
}

std::vector<std::uint8_t> Ppmd8Backend::decode(
    const ByteView payload,
    const std::size_t expected_size) const {
    require_valid_view(payload, "PPMd8 decoder");
    if (payload.size() < kPayloadHeaderSize + 4U) {
        throw std::runtime_error("Truncated PPMd8 payload");
    }
    if (!std::equal(kPayloadMagic.begin(), kPayloadMagic.end(),
                    payload.data())) {
        throw std::runtime_error("Invalid PPMd8 payload magic");
    }
    if (payload[4] != kPayloadVersion || payload[5] != kPayloadFlags ||
        payload[kEnvelopeSizeOffset] != kPayloadHeaderSize) {
        throw std::runtime_error("Unsupported PPMd8 payload format");
    }
    for (std::size_t index = kReservedOffset;
         index < kPayloadHeaderSize; ++index) {
        if (payload[index] != 0) {
            throw std::runtime_error("Invalid PPMd8 payload reserved bytes");
        }
    }

    const unsigned order = payload[6];
    const unsigned restore_method = payload[7];
    const std::uint32_t memory_size =
        read_u32_le(payload, kMemorySizeOffset);
    try {
        validate_profile(order, memory_size, restore_method);
    } catch (const std::invalid_argument&) {
        throw std::runtime_error("Unsupported PPMd8 payload profile");
    }
    ...
    if (declared_size != expected_size) {
        throw std::runtime_error("PPMd8 output size does not match metadata");
    }
    if (declared_size > maximum_output_size_) {
        throw std::length_error("PPMd8 output exceeds configured block limit");
    }
    if (compressed_size != payload.size() - kPayloadHeaderSize) {
        throw std::runtime_error("PPMd8 stream size does not match payload");
    }
    const ByteView compressed(payload.data() + kPayloadHeaderSize,
                              compressed_size);
    if (crc32(compressed) != read_u32_le(payload, kCompressedCrcOffset)) {
        throw std::runtime_error("PPMd8 compressed payload checksum mismatch");
    }

    Ppmd8Model model(memory_size);
    BoundedByteIn stream{};
    stream.vt.Read = read_byte;
    stream.data = compressed.data();
    stream.size = compressed.size();
    model.get()->Stream.In = &stream.vt;
    if (!Ppmd8_Init_RangeDec(model.get()) || stream.exhausted) {
        throw std::runtime_error("Invalid PPMd8 range stream header");
    }
    Ppmd8_Init(model.get(), order, restore_method);

    std::vector<std::uint8_t> output(declared_size);
    for (std::size_t index = 0; index < declared_size; ++index) {
        const int symbol = Ppmd8_DecodeSymbol(model.get());
        if (stream.exhausted || symbol < 0 || symbol > 255) {
            throw std::runtime_error("PPMd8 range stream ended before output");
        }
        output[index] = static_cast<std::uint8_t>(symbol);
    }
    const int end_symbol = Ppmd8_DecodeSymbol(model.get());
    if (stream.exhausted || end_symbol != PPMD8_SYM_END ||
        !Ppmd8_RangeDec_IsFinishedOK(model.get()) ||
        stream.position != stream.size) {
        throw std::runtime_error("PPMd8 range stream did not finish exactly");
    }
    if (crc32(ByteView(output)) !=
        read_u32_le(payload, kUncompressedCrcOffset)) {
        throw std::runtime_error("PPMd8 output checksum mismatch");
    }
    return output;
}

std::size_t Ppmd8Backend::maximum_payload_size(
    const std::size_t input_size) {
    return checked_add(kPayloadHeaderSize, stream_capacity_for(input_size),
                       "PPMd8 payload bound overflow");
}
