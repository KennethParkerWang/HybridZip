...
class Ppmd8Backend final : public IBlockEntropyBackend {
public:
    static constexpr std::size_t kPayloadHeaderSize = 32;
    static constexpr unsigned kDefaultOrder = 8;
    static constexpr unsigned kDefaultRestoreMethod = 1;  // CUT_OFF
    static constexpr std::uint32_t kMinimumMemorySize =
        1U * 1024U * 1024U;
    ...
    explicit Ppmd8Backend(
        unsigned order = kDefaultOrder,
        std::uint32_t memory_size = kDefaultMemorySize,
        unsigned restore_method = kDefaultRestoreMethod,
        std::size_t maximum_output_size = kDefaultMaximumOutputSize);

    const char* name() const noexcept override { return "ppmd8-7zip"; }
    EntropyKind kind() const noexcept override { return EntropyKind::Ppmd8; }
